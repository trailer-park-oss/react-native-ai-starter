...
function getProjectNameFromPackageJson(projectRoot) {
  const packageJsonPath = path.join(projectRoot, 'package.json')
  if (!fs.existsSync(packageJsonPath)) {
    return null
  }
...
function requiresReactNative076Compatibility(packageJson) {
...
function patchExpoHeadPodspec(content) {
...
function patchExpoPodspec(content) {
